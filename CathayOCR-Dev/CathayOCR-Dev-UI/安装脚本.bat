...
:: 查找启动脚本（支持不同命名）
set "BAT_PATH=%SCRIPT_DIR%\启动 CathayOCR Dev.bat"
if not exist "%BAT_PATH%" set "BAT_PATH=%SCRIPT_DIR%\开启 CathayOCR Dev.bat"
if not exist "%BAT_PATH%" (
    echo [错误] 找不到启动 .bat 文件：
    echo   期望: 启动 CathayOCR Dev.bat
    echo   或:   开启 CathayOCR Dev.bat
    echo 请确认 CathayOCR Dev 已正确安装到插件目录。
    pause
    exit /b 1
)
...
set "SHORTCUT_NAME=CathayOCR Dev.lnk"
...
powershell -NoProfile -Command ^
    ...
    "$s.Description = 'CathayOCR Dev - GPU加速PDF处理器'; " ^
...
set "STARTMENU_DIR=%STARTMENU%\CathayOCR Dev"
...
echo.
echo  运行前请确保：
echo    1. Umi-OCR Paddle v2.1.5 已安装
