...
:: 查找启动脚本（支持不同命名）
set "BAT_PATH=%SCRIPT_DIR%\启动 CathayOCR Lite.bat"
if not exist "%BAT_PATH%" set "BAT_PATH=%SCRIPT_DIR%\开启 CathayOCR Lite.bat"
if not exist "%BAT_PATH%" (
    echo [错误] 找不到启动 .bat 文件：
    echo   期望: 启动 CathayOCR Lite.bat
    echo   或:   开启 CathayOCR Lite.bat
    echo 请确认 CathayOCR Lite 已正确安装到插件目录。
    pause
    exit /b 1
)
...
set "SHORTCUT_NAME=CathayOCR Lite.lnk"
...
powershell -NoProfile -Command ^
    ...
    "$s.Description = 'CathayOCR Lite - GPU加速PDF处理器'; " ^
...
set "STARTMENU_DIR=%STARTMENU%\CathayOCR Lite"
