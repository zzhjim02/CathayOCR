...
**打开安装包 → 双击启动 → 拖入 PDF → 开始处理，几分钟后拿到整洁的电子文本。**
...
| 你的身份 | 下载这个 | 理由 |
|:--------|:--------|:------|
| 🟢 **普通用户**——只想用工具识别 PDF | **Lite 轻量版** | 630 MB，解压即用，够快够好用 |
| 🔵 **重度用户**——需要最高精度/最多语言 | **Pro 专业版** | 6引擎全量 + CUDA GPU加速 |
| 🟣 **开发者**——想研究/修改代码 | **DEV 开发版** | 完整 Python + C++ 源码，全套开发环境 |
| 🟡 **想自己改代码但不需开发环境** | 任何一个安装包 | 安装包内的 .py 文件直接用记事本/任何编辑器就能改 |
...
> 💡 先下 Lite 版试试，觉得好用再升级 Pro。Lite 版能覆盖 90% 的使用场景。

### Pro 专业版（~5.7 GB .zip | 解压 ~9 GB）

六引擎全量，最高精度。适合学者、重度用户。
...
<b>选 Lite 版就够了。</b>Lite 版用 ncnn Vulkan 引擎，支持任意品牌 GPU 加速，没显卡也能跑 CPU 模式。Pro 版多了 CUDA、EasyOCR 和 Paddle 备胎引擎，适合需要阿拉伯文/天城文识别，或想多引擎对比的用户。可以先下 Lite 版试试，好用再升 Pro。
</details>
...
| [ppocr-ncnn-cpp](https://github.com/CrumpetTurtle/ppocr-ncnn-cpp) | ncnn 版 PP-OCR 引擎的 C++ 实现（本项目核心引擎源码基此二次开发） | — |
