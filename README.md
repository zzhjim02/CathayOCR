...
> 💡 **在 CPU 架构极老的云服务器上也能用**：如果遇到设备没有 Vulkan 驱动、没有 CUDA 支持、CPU 型号老旧（如云服务器），Pro 版还有 PP-OCRv5 引擎可用。只需在界面中选「PP-OCRv5 Paddle CPU」即可跑起来，速度虽慢但至少能用。Pro 版里不需要的引擎文件夹也可以直接删除，完全不影响使用。
